...
<page>
	<div style="margin-left:30px; margin-right:30px; padding-top:30px">
		...
		<h3>Reporte Egresos </h3>
		<h1 style="margin-top:40px; text-align:center">TOP MKT<br>EGRESOS</h1>
		<div style="margin-left:60px; margin-right:60px">
					...
					while($row= mysqli_fetch_assoc($query)){
						...
						$subconsulta =
							'SELECT
							    ...
							    INNER JOIN finanzas_pagos ON finanzas_pago_solicitud.id_pago_solicitud = finanzas_pagos.id_pago_solicitud
							    WHERE
							        finanzas_pagos.id_subconcepto_pago = cat_subconcepto_pagos.id AND finanzas_pago_solicitud.status = 3 AND finanzas_pagos.fecha_deposito LIKE "'.$fecha.'%"
									) AS monto
							FROM
							    finanzas_pagos
							INNER JOIN cat_subconcepto_pagos ON finanzas_pagos.id_subconcepto_pago = cat_subconcepto_pagos.id
							WHERE
			...
			<table style="width:100%; padding-top:45px; padding-left:15px">
				<tr>
					<th style='width:60%; font-size: 18px; '>Total Egresos</th>
