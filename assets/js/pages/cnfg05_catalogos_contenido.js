  ...
  $.ajax({
    ...
    success: function (data) {
      if (data == "error") {
        $("#message").empty();
        $("#message").append(
          '<div class="alert alert-danger"><a href="#" class="close" data-dismiss="alert">&times;</a>La tabla seleccionada no se ha agregado a la base de datos.</strong></div>'
        );
      }else{
        if(b>0){
        ...
        document.getElementById("btn-nuevo").removeAttribute("disabled")
        $("#message").empty();
        $("#tabla").append(data);
        dataTab('#tabla');
      }
