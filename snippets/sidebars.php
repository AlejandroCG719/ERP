...
<nav id="page-leftbar" role="navigation">
  ...
  <ul class="acc-menu" id="sidebar">
    ...
    <?php
      ...
      $consulta =
        'SELECT DISTINCT
          supermodulos.id_supermodulo,
          supermodulos.descripcion,
          supermodulos.nombre,
          supermodulos.imagen
        FROM supermodulos
          INNER JOIN modulos ON modulos.id_supermodulo=supermodulos.id_supermodulo
          INNER JOIN modulo_has_perfiles ON modulo_has_perfiles.id_modulo=modulos.id_modulo
        WHERE
          modulo_has_perfiles.id_perfil='.$id_perfil.' ORDER BY supermodulos.id_supermodulo ASC
        ;'
      ;
      ...
      if(mysqli_fetch_assoc($query)>0){
        ...
        while($row= mysqli_fetch_assoc($query)){
          ?>
             <li class="hasChild">
               <a href="#" title="<?= $row['descripcion'] ?>">
                <i class="<?= $row['imagen'] ?>"></i><span><?= $row['nombre']	 ?></span>
              </a>
              <ul class="acc-menu" style="<?= (8 == $row['id_supermodulo']) ? 'display: none;' : '' ?>">
                <?php
                  $subconsulta =
                    'SELECT DISTINCT
                      modulos.id_modulo,
                    ...
                    FROM
                      modulos
                      INNER JOIN modulo_has_perfiles ON modulo_has_perfiles.id_modulo=modulos.id_modulo
                    WHERE modulo_has_perfiles.id_perfil='.$id_perfil.' AND id_supermodulo='.$row['id_supermodulo'].'
                      ORDER BY modulos.id_modulo ASC
                    ;'
                  ;
                  $subquery=mysqli_query($conexion,$subconsulta) or die(mysql_error());
                  if(mysqli_fetch_assoc($subquery)>0){
                    $subquery=mysqli_query($conexion,$subconsulta) or die(mysql_error());
                    while($rowsub= mysqli_fetch_assoc($subquery)){
                      ?>
                        <li>
                          <a href="<?= $rowsub['liga'] ?>" title="<?= $rowsub['descripcion'] ?>">
                            <i class="<?= $rowsub['imagen'] ?>"> </i><span><?= $rowsub['nombre'] ?></span>
                          </a>
                        </li>
                      <?php
                    }
